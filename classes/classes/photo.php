...
class photo{
	...
	function view_folder($key,$id){
		...
		*/
		...
		if ($topic->loadTournFromTimestamp($key)){
			...
			$this->db=new DB;
			$this->db->query(sprintf('select * from %s where ID like "%s"',$topic->t_name,$id));
			$this->db->next_record();
			
			
			echo '<b>Photo Album for '.$this->db->f('title').'</b><br><br>';
			echo '<a href="'.$_SERVER['PHP_SELF'].'?function=tourn_view&id='.$id.'">Back</a><hr>';
			...
			foreach($topic->items as $name=>$rawfile){
				...
				if ( $file != "." && $file != ".." ) {
					...
					if ( $accepted_types[$extension] ) {
						...
						//						echo '<a href='.$_SERVER['PHP_SELF']."?function=show&key=".$key."&id=".$id."&file=".str_replace ('&', '%26', str_replace (' ', '%20', $file)).'>';
						
						if ( $thumbnails && ($accepted_types[$extension]=='y') ) {
							@ $size = GetImageSize ($thumbnail_directory_name.$thumbnail_prefix.$file);
							echo sprintf('<table width=80><tr height=80><td colspan=3><center><a href=%s?function=show&key=%s&id=%s&file=sm_%s><img src="%s" border=0 %s></a></center></td></tr><tr>',$_SERVER['PHP_SELF'],$key,$id,str_replace ('&', '%26', str_replace (' ', '%20', $file)),$imagepath.$key.'/'.$thumbnail_prefix.$file,$size[3]);
							
							@ $size = GetImageSize ($thumbnail_directory_name.'sm_'.$file);
							echo sprintf('<td><center><a href=%s?function=show&key=%s&id=%s&file=sm_%s>Small</a><br><font style="font-size:9px">(%sx%s)<br><nobr>%s</nobr></font></center></td>',$_SERVER['PHP_SELF'],$key,$id,str_replace ('&', '%26', str_replace (' ', '%20', $file)),$size[0],$size[1],$this->fsize($thumbnail_directory_name.'sm_'.$file));
							
							@ $size = GetImageSize ($thumbnail_directory_name.'md_'.$file);
							echo sprintf('<td><center><a href=%s?function=show&key=%s&id=%s&file=md_%s>Medium</a><br><font style="font-size:9px">(%sx%s)<br><nobr>%s</nobr></font></center></td>',$_SERVER['PHP_SELF'],$key,$id,str_replace ('&', '%26', str_replace (' ', '%20', $file)),$size[0],$size[1],$this->fsize($thumbnail_directory_name.'md_'.$file));
							
							@ $size = GetImageSize ($thumbnail_directory_name.$file);
							echo sprintf('<td><center><a href=%s?function=show&key=%s&id=%s&file=%s>Large</a><br><font style="font-size:9px">(%sx%s)<br><nobr>%s</nobr></font></center></td></tr>',$_SERVER['PHP_SELF'],$key,$id,str_replace ('&', '%26', str_replace (' ', '%20', $file)),$size[0],$size[1],$this->fsize($thumbnail_directory_name.$file));
							
						}else if ( $thumbnails ) {
							echo sprintf('<table width=80><tr height=80><td colspan=3><a href=%s?function=show&key=%s&id=%s&file=%s><img src="%s" border=0></a></td></tr>',$_SERVER['PHP_SELF'],$key,$id,str_replace ('&', '%26', str_replace (' ', '%20', $file)),$webpath.'fileicons/'.$accepted_types[$extension]);
						}
	...
	function view_media($key, $file){
		...
		echo '<a href="'.$_SERVER['PHP_SELF'].'?function=photo&id='.$id.'&key='.$key.'">Back</a><br><br>';
		...
		if ( $types_image[$extension] ) {
			...
			while($this->db->next_record()){
				if (strpos($file,$this->db->f('file'))){
					...
					echo sprintf('<table><tr><td><center><a href=%s?function=show&key=%s&id=%s&file=sm_%s>Small</a><br><font style="font-size:9px">(%sx%s)<br><nobr>%s</nobr></font></center></td>',$_SERVER['PHP_SELF'],$key,$id,str_replace ('&', '%26', str_replace (' ', '%20', $file)),$size[0],$size[1],$this->fsize($path.'sm_'.$file));
					
					@ $size = GetImageSize ($path.'md_'.$file);
					echo sprintf('<td><center><a href=%s?function=show&key=%s&id=%s&file=md_%s>Medium</a><br><font style="font-size:9px">(%sx%s)<br><nobr>%s</nobr></font></center></td>',$_SERVER['PHP_SELF'],$key,$id,str_replace ('&', '%26', str_replace (' ', '%20', $file)),$size[0],$size[1],$this->fsize($path.'md_'.$file));
					
					@ $size = GetImageSize ($path.$file);
					echo sprintf('<td><center><a href=%s?function=show&key=%s&id=%s&file=%s>Large</a><br><font style="font-size:9px">(%sx%s)<br><nobr>%s</nobr></font></center></td></tr>',$_SERVER['PHP_SELF'],$key,$id,str_replace ('&', '%26', str_replace (' ', '%20', $file)),$size[0],$size[1],$this->fsize($path.$file));
