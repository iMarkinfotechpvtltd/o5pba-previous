...
class usermanager{
	...
	function adduser($user){
		$this->db->query(sprintf('INSERT INTO auth VALUES("%s","%s","%s","%s")',md5(uniqid('mountthefloopy')),$user->name,md5($user->pass),$user->perms));
	...
	function changepass($id,$password){
		$this->db->query(sprintf('UPDATE auth SET PASSWORD="%s" WHERE user_id="%s"',md5($password),$id));
	...
	function auser($thisid=''){
		...
		if($thisid!=''){
			...
			if($this->db->num_rows()==1){
				...
				$this->perms = explode(',',$this->db->f('perms'));
			}
		}
	}
}
