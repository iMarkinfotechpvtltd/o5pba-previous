...
class topicsystem{
	...
	function topicsystem($name,$section){
		...
		$this->db = new DB;
		$this->postfiles = $htdocsdir.''.$name.'/posts/';
		$this->abs=$htdocsdir;
	}
	
	function addTopic($topic){
		global $auth;
		$this->db->query(sprintf('INSERT INTO %s (created,title,modified,modifiedby,createdby) VALUES("%s","%s","%s","%s","%s")',$this->t_name,$topic->created,$topic->title,$topic->created,$auth->auth['uname'],$auth->auth['uname']));
		$test = true;
		if($this->db->affected_rows()==1){
			logit("Topic '".$topic->title." added to ".$this->system_name);
			foreach($topic->items as $title=>$data){
				$this->db->query(sprintf('INSERT INTO %s (created,ititle,file,order1) VALUES("%s","%s","%s","%s")',$this->t_name.'_items',$topic->created,$title,$data[0],$data[2]));
	...
	function updateTopic($topic){
		...
		if($this->db->num_rows()==1){
			$this->db->query(sprintf('UPDATE %s SET title="%s",modified="%s",modifiedby="%s" WHERE created="%s" ',$this->t_name,$topic->title,$newtime,$auth->auth['uname'],$topic->created));
			...
			foreach($topic->items as $name => $data){
				if(isset($data[3])){
					...
					if ($this->db->next_record()){
						$this->db->query(sprintf('UPDATE %s_items SET ititle="%s",order1="%s",discription="%s" WHERE created="%s" AND order1="%s"',$this->t_name,$name,$count, $_POST['item_description_'.$count],$topic->created,$data[2]));
					}else{
						$this->db->query(sprintf('INSERT INTO %s_items (ititle,order1, created) values ("%s","%s","%s")',$this->t_name,$name,$count,$topic->created));
					...
					if($this->db->num_rows()!=0){
						...
						$this->db->query(sprintf('UPDATE %s_items SET ititle="%s",file="%s",order1="%s",discription="%s",modified="%s" WHERE created="%s" AND order1="%s"',$this->t_name,$name,$data[0],$count,$_POST['item_description_'.$count],time(),$topic->created,$count));
						$count++;
					}else{
						if (!is_dir($this->postfiles.$topic->created.'/')){
							mkdir($this->postfiles.$topic->created.'/', 0777);
						...
						$this->db->query(sprintf('INSERT INTO %s_items (created,modified,ititle,file,order1,discription) values("%s","%s","%s","%s","%s","%s")',$this->t_name,$topic->created,time(),$name,$data[0],$count,$_POST['item_description_'.$count]));
						$count++;
					}
				}
			}
			...
			if(isset($data[3])){
				...
				if ($this->db->next_record()){
					if ($this->db->f('ititle')!=$name || $this->db->f('order1')!=$count || $this->db->f('discription')!=$_POST['item_description_'.$count]){
						$this->db->query(sprintf('UPDATE %s_items SET ititle="%s",order1="%s",discription="%s",modified="%s" WHERE created="%s" AND order1="%s"',$this->t_name,$name,$count,$_POST['item_description_'.$count],time(),$topic->created,$data[2]));
					}
				}else{
					$this->db->query(sprintf('INSERT INTO %s_items (ititle,order1, created, modified) values ("%s","%s","%s","%s")',$this->t_name,$name,$count,$topic->created,time()));
	...
	function iterate(){
		$this->it_db = new DB;
		$this->it_db->query(sprintf('SELECT * FROM %s ORDER BY created DESC',$this->t_name));
	}
...
class topic{
	...
	function loadFromTimestamp($stamp){
		//echo(sprintf('SELECT t.*,i.order1 as order1,i.file as file,i.ititle as ititle FROM %s t,%s i WHERE t.created="%s" AND i.created="%s" ORDER BY order1<Br>',$this->t_name,$this->t_name.'_items',$stamp,$stamp));
