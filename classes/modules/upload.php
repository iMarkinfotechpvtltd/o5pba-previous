...
class Upload_Files {
	...
	****************************************************************************/
	...
	function averagebook_uploadform_old(){
		?>
		<SCRIPT LANGUAGE="JavaScript">
		function disableForm(theform) {
			if (document.all || document.getElementById) {
		...
		echo '<tr><td style="text-align: right">Image file:</td><td style="text-align: left">';
		$this->form->show_element('upload');
		echo '<br><font style="font-size:8pt;">Notes:<br>';
      echo 'The CSV format is supported and will merge identical records. The uploaded CSV file must have the field names in the first row ';
      echo 'and must contain the correct field names in the correct order.  If you upload an invalid file the correct headers will be displayed.  CSV files must end with ';
      echo 'the .csv extension.<br/>';
      echo '</font></td></tr>';
		echo '<tr><td>&nbsp;</td><td style="text-align: left">';
		$this->form->show_element('action');
	...
	function average_book_upload_form() {
		?>
		<SCRIPT LANGUAGE="JavaScript">
		function disableForm(theform) {
			//if (!doValidation(theform) || !doConfirmation())
				//return false;
		...
		$this->form->start('disableForm','POST',$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'],"' onSubmit='return disableForm(this);' autocomplete='off'",'form');
		echo '<table class="listing">';
