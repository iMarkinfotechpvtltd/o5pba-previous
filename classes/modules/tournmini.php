...
class tournmini{
	...
	var $titles;
	var $postpath;
	var $formatfile='tournmini.ihtml';
	...
	function tournmini($t_name, $name=''){
		global $id,$htdocsdir;
		$this->t_name = $t_name;
		$this->titles= $name;
	...
	function edit($id){
		...
		$this->db->next_record();
		$this->tournmini($this->t_name, $this->db->f('title'));
		$this->showform();
	...
	function listitems($key,$title=''){
		...
		echo '[<a href="'.$_SERVER['PHP_SELF'].'?function=tournament_edit&time='.$key.'">Edit</a>]<br>';
		echo '<table><tr><td class="" width=250>';
...
class viewtournmini{
	...
	function viewtournmini($t_name, $name=''){
		global $id;
		$this->t_name = $t_name;
	...
	function listing($hideyears=0, $color=''){
		...
		*/
		...
		if ($this->db->num_rows()){
			while($this->db->next_record()){
				...
				echo sprintf('<td width=350><nobr><a href="index2.php?function=tourn_view&id=%s">%s</a></nobr></td>',$this->db->f('ID'),$this->db->f('title'));
				
			}
		}else{
			//echo sprintf('<tr><td colspan="3">No %s found!</td></tr>',$this->titles);
		}
		...
		if ($topic->loadTournFromTimestamp($key)){
			...
			foreach($topic->items as $name=>$file){
				if ($file[0]=='ComingSoon.pdf'){
					...
					echo sprintf('&bull;&nbsp;%s%s<br>',linkit($webpath.$this->postpath.$key.'/'.$file[0],$name), $suffix);//date('M d/Y  @ H:i',$file[4])
				}
			}
			echo '</td></tr></table>';
		}else{
	...
	function photoitems($key, $title=''){
		...
		if ($topic->loadTournFromTimestamp($key)){
			...
			$suffix = "";
			if ($duration<(7*24*60*60)) $suffix = "  <font style='background: #ff3300; color: #ffffff'>&nbsp;updated&nbsp;</font>";
			echo sprintf('<table><tr><td width="20"></td><td valign="middle">&bull;&nbsp;<a href="javascript:albumPopup(\'../bowling_school_mini/album.php?key=%s&id=%s\');">View photo album</a>%s</td></tr></table>',$key,$idstr, $suffix);
		...
		if (hasmodule('advancedphoto')){
			...
			*/
			...
			if ($topic->loadTournFromTimestamp($key)){
				...
				echo "<table border=0 cellpadding=2 width=600><tr>";
