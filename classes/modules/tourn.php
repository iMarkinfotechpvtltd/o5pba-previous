...
class tournaments{
	...
	function listing(){
		...
		echo '<tr><td colspan="6"><table width="100%"><tr>';
	...
	function listitems($key,$title=''){
		...
		echo $title.' [<a href="'.$_SERVER['PHP_SELF'].'?function=tournament_edit&time='.$key.'">Edit</a>]<br>';
		echo '<table><tr><td class="w50">';
		...
		echo '</td><td width="50" align="center" style="text-align:center;vertical-align:middle;"></td></tr></table>';
	}
...
function edititems($time, $ret_to_code=''){
	// $time = $_POST['time'];
	if(isset($_POST['time']))
	{
	$time = $_POST['time'];	
	}
	global $ts,$htdocsdir,$f,$numitems;
	if(isset($_GET['time']))
	{
	$time = $_GET['time'];	
	}
				...
				// if($test=='ok'){
					...
					if($filetest){
						if ($ret_to_code) {
							...
							echo "<a href='bowling_school.php'>Go back to Bowling School listing</a>";
						} else {
...
<center>
".$this->getSeason(substr($time,0,strpos($time,'_')))." Season
</center>
</td>
</tr></table>";
...
function view($id){
	...
	$this->db->query(sprintf('select * from %s where ID like "%s"',$this->t_name,$id));
	$this->db->next_record();
	$season = $this->getSeason($this->db->f('created'));
...
class viewtournaments{
	...
	function listing($hideyears=0){
		...
		echo '<th colspan=3>'.$this->titles.'</th>';
		echo '<tr><td colspan="3" class="header_dates" nowrap="nowrap" ><table cellpadding="2"><tr>';
		...
		if ($this->db->num_rows()){
			while($this->db->next_record()){
				...
				echo sprintf("<td  nowrap='nowrap'>%s</td>",date("M d, Y",$this->db->f('modified')));
				
				echo sprintf('<td nowrap="nowrap"><nobr><a href="?function=tourn_view&id=%s">%s</a></nobr></td>',$this->db->f('ID'),$this->db->f('title'));
				echo "</tr>";
			}
		}else{
	...
	function listitems($key, $title=''){
		...
		if ($topic->loadTournFromTimestamp($key)){
			echo $title;
			echo '<table><tr><td width=20></td><td width=450>';
			foreach($topic->items as $name=>$file){
				if ($file[0]=='ComingSoon.pdf'){
					echo sprintf('&bull;&nbsp;%s - Coming Soon!<br>',$name);
				}else{
					$duration = time() - $file[4];
					$suffix = "";
					if ($duration<(7*24*60*60)) $suffix = "  <font style='background: #ff3300; color: #ffffff'>&nbsp;updated&nbsp;".date('m/d  @ H:i',$file[4])."</font>";
					echo sprintf('&bull;&nbsp;%s%s<br>',linkit($webpath.$this->postpath.$key.'/'.$file[0],$name), $suffix);
				}
			}
			echo '</td></tr></table>';
		}else{
	...
	function resultstitle($key, $table=''){
		$topic=new topic($this->t_name);
		...
		if (($results) && ($minutes)){
			echo $table.'<font size="2">Minutes / Results:';
		}elseif($minutes){
			echo $table.'<font size="2">Minutes:';
		}elseif($results){
			echo $table.'<font size="2">Results:';
		}else{
			echo '&nbsp;>';
	...
	function photoitems($key, $title=''){
		...
		if ($topic->loadTournFromTimestamp($key)){
			...
			function albumPopup(url)
			{
			testwindow= window.open (url, "mywindow",
    			"location=1,resizable=1,status=1,scrollbars=1,width=850,height=600");
			}
			</SCRIPT>';
			
			echo $title;
			$idstr = isset($id)? $id : $_GET['expand'];
			$this->db->query(sprintf('select modified from %s_items where created="%s" order by modified DESC',$this->t_name,$key));
			$this->db->next_record();
			$duration = time() - $this->db->f('modified');
			$suffix = "";
			if ($duration<(7*24*60*60)) $suffix = "  <font style='background: #ff3300; color: #ffffff'>&nbsp;updated&nbsp;</font>";
			echo sprintf('<table><tr><td width="20"></td><td valign="middle">&bull;&nbsp;<a href="javascript:albumPopup(\'album.php?key=%s&id=%s\');">View photo album</a>%s</td></tr></table>',$key,$idstr, $suffix);
		...
		if (hasmodule('advancedphoto')){
			...
			*/
			...
			if ($topic->loadTournFromTimestamp($key)){
				...
				echo "<table border=01 cellpadding=2 width=1000><tr>";
