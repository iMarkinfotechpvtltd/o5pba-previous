...
$ts = new topicsystem('coach','Coach\'s Corner');
$ts->postfiles=$htdocsdir.'coach/posts/';
echo "<pre>";
print_r($_GET);
echo "</pre>";
echo "<pre>";
...
function display_details($topic){
	...
	foreach($items as $title=>$data){
    	echo '<li>'.$title.'(<a href="/coach/posts/'.$topic->created.'/'.$data[0].'">'.$data[0].'</a>)</li>'."\n";
    }
    echo '</ul>';
}
//Draw Forms
	...
	if($function == 'addtopic'){
	  ...
	  <table width="500">
	  	<tr>
	  		<td>
	  			<center>
	  			<? echo getSeason('coach', $_GET['time']);  ?> Season
	  			</center>
	  		</td>
	  	</tr>
	  </table>
