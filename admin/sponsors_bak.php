	...
	function validate(){
		...
		if($this->form->validate()){
			...
			if ($_POST['company_name'] == '') {
				...
				echo "<font color='red'>Please input the link address !</font>"; 
				return;
			...
			if ($_POST['id']!=''){
				...
				if ($_POST['new_sponsorship_level'] != ''){
					...
					$query = sprintf('insert into sponsors ( logo, url, company_name, contact_name, sponsorship_level) VALUES ("%s","%s","%s","%s", "%s");',$_FILES["logo"]["name"], $_POST['url'],$_POST['company_name'],$_POST['contact_name'], $new_sponsorship_id);
					$this->db->query($query);
				} else {
					$query = sprintf('insert into sponsors ( logo, url, company_name, contact_name, sponsorship_level) VALUES ("%s","%s","%s","%s", "%s");',$_FILES["logo"]["name"], $_POST['url'],$_POST['company_name'],$_POST['contact_name'], $_POST['sponsorship_level']);
					$this->db->query($query);
				}
			}
//			echo $query;
	...
	function plist($id=''){
		...
		echo '<tr><td colspan=1></td><th width=150>Logo</th><th width=150>Company_Name</th><th width=150>Contact_Name</th><th width=150>URL</th><th width=150>Sponsonship_Level</th><td></td></tr><tr><td colspan=7>&nbsp;</td></tr>';
		$total=0;
		while($this->db->next_record()){
