...
if(isset($_POST['date']))
{
setcookie("TestCookie",$_POST['date']);
}
...
//$time = strtotime("1 January ".$_GET['nyear']);
echo "<pre>";
print_r($_POST);
echo "</pre>";
if(isset($_POST['ask_numitems']))
{
	$ask_numitems = $_GET['ask_numitems'];
}
if(isset($_POST['date']))
{
	$date = $_POST['date'];
...
if($_GET['nyear']!=''){
echo $tempo = date("d M ",time()).$_GET['nyear'] ;
}else{
echo $tempo = date("d M Y",time());
}
...
$f->add_element(array("type"=>"text","name"=>"date","size"=>8,"maxlength"=>"11","value"=>"$tempo"));
...
function display_details($topic){
	...
	foreach($items as $title=>$data){
    	echo '<li>'.$title.'(<a href="/bowling_school/posts/'.$topic->created.'/'.str_replace("+", "%20", urlencode($data[0])).'">'.$data[0].'</a>)</li>'."\n";
    }
    echo '</ul>';
}
//Draw Forms
function ask_numitems(){
	...
	$f->start('','', $_SERVER['PHP_SELF'].'?function='.$function.'&nyear='.$_GET[nyear]);
	?>
	<table width="500" cellpadding="3" cellspacing="0" border="0" bgcolor="#AFAFAF">
		<tr bgcolor="#AFAFAF">
	    <td>How many items to add to this post? <? $f->show_element('ask_numitems');?></td></tr>
	    <tr bgcolor="#AFAFAF">
	    <td>what date should these be set to? <? $f->show_element('date');?></td>
	    </tr>
	    <tr bgcolor="#CFCFCF">
	...
	if($function == 'addtopic'){
	   ...
	   <table width="500">
	  	<tr>
	  		<td>
	  			<center>
	  			<? echo getSeason('bownling_school', $_GET['time']);  ?> Season
	  			</center>
	  		</td>
	  	</tr>
	  </table>
...
switch($function){
	...
	default:
        ...
        ?>
        <tr><td colspan="4"><a href="<?php echo $_SERVER['PHP_SELF']."?function=asknumitems&nyear=$_GET[nyear]"?>">Add Topic</a><br><br></td></tr>        
        <tr>
        	<td width="50" align="center" style="text-align:center;vertical-align:middle;">
        		<a href="bowling_school_results.php">Edit</a>
        	</td>
        	<td>Aug 23, 2006</td>
        	<td>
        		<a href="bowling_school_results.php">Edit Results / Pictures</a>
        	</td>
        	<td width="50" align="center" style="text-align:center;vertical-align:middle;">
        	</td>
        </tr>
        <tr>
        	<td width="50" align="center" style="text-align:center;vertical-align:middle;">
        		<a href="bod.php?class=bowl">Edit</a>
        	</td>
        	<td>Aug 23, 2006</td>
        	<td>
        		<a href="bod.php?class=bowl">Edit Staff / Instructors / Pros</a>
        	</td>
        	<td width="50" align="center" style="text-align:center;vertical-align:middle;">
        	</td>
        </tr>
	...
	case 'addtopic':
		if((isset($_POST['submit']))&&($_POST['numitems']!='0')){
			...
			// if($test=='ok'){
				...
				$topic->created = strtotime("$_COOKIE[TestCookie]");}
				else{
				$topic->created = time();}
				//echo $topic->created;
				for($i=0;$i<$numitems;$i++){
