	 ...
	 $failure=0;
		  while (!feof($fp)) {
		   ...
		   if($headers ==0 && $row[0] > 0)
		   {
		    ...
		    if($headers == 1) 
		   { 
		      for($i = 0 ; $i < 17 ; $i++) // getting rid of quotations
			  {
        ...
        // for ($dd=0;$dd<13;$dd++) {echo $dd.': '.$row[$dd].'<br />';}
        // exit;
				// echo $row[2];
				if($row[12])//zone
				{
				   
					$db2->query("select id from oa_zones where name = '$row[12]'");
					if($db2->next_record())
					{
					    $zone = $db2->f("id");
					//  echo $zone;
				...
				if($row[1] && $add == 1)//centre
				{
					$db2->query("select id from oa_centres where name = '$row[1]' and zone_id = $zone");
					if($db2->next_record())
					{
					   $centre = $db2->f("id");
					  // echo $db2->f("id");
				...
				if($row[2] && $add == 1)//league name
				{
				   
					$db2->query("select id from oa_leagues where  name = '$row[2]' and centre_id = $centre");
					if($db2->next_record())
					{
					   $league = $db2->f("id");
				...
				if(!$row[5] && $add == 1)//check c5#
				{
				   // c5# found don't need to create new member
					$db2->query("select id from oa_members where  member_number ='$row[8]'");
					if($db2->next_record())
					{
					   $member = $db2->f("id");
				...
				else if($add == 1) // create new member
				{
				  ...
				  $db2->query("select * from oa_members where member_number = $row[5]");
				  if($db2->next_record()){
				  ...
				  else if($add == 1 && $_POST['rep'] == "y")
				  {
					...
					echo "adding new member c5#: $member_number First_name: $row[4] last_name: $row[3] Gender: $gender <br/>";
			...
			$row[13] =str_replace("/","-", $row[13]);
;
				    $db2->query("select id from oa_seasons where name = '$row[13]'");
					if($db2->next_record())
					{
						$season = $db2->f('id');
...
function average_book_index() {
	...
	echo "<table><tr><td><p style='font-size:150%'>Beginning of season - Member registration</p></td><td valign='bottom'><input type='radio' id='rep' name='rep' value='y'/></td></tr>";
	echo "<tr> <td><p style='font-size:150%'>End of Season - Update Members bowling stats (games and Pinfall)</p> </td><td valign='bottom'><input type='radio' id='rep' name='rep' value='n' checked/></td></tr>";
	echo "</table><br/> <input type='file' name='file' id='file'/><br/>";
...
function edit_member_bowlings_form() {
	...
	if ($_GET['lid']) {
		$where .= sprintf(' AND l.id = %d', $lid);
	}
	if ($_GET['cid']) {
		$where .= sprintf(' AND c.id = %d', $cid);
	}
	if ($_GET['zid']) {
		$where .= sprintf(' AND z.id = %d', $zid);
	}
	if ($first_name) {
		$where .= sprintf(' AND m.first_name LIKE "%%%s%%"', $first_name);
	}
	...
	if ($membership_number) {
		$where .= sprintf(' AND CONCAT_WS(\'\', z.code, m.member_number, CONCAT_WS(\'-\', c.code, l.number)) LIKE "%%%s%%"', $membership_number);
	}
