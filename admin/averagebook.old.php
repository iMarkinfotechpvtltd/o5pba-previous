 ...
 */
$expectedColumns = array('Association','Membership#','Last Name','First Name','Category','Sex','League_Codes','Last Year','Total Games Played','Running Average', 'Ont Place', 'Assoc Place',  'GP_09', 'AV_09', 'GP_08', 'AV_08', 'GP_07', 'AV_07', 'GP_06', 'AV_06', 'GP_05', 'AV_05');
...
if ($_POST['action']=='archive'){
   ...
   $avgcols = array('first','last','memberid','average','games','league','association', 'year', 'category', 'sex', 'oplacing', 'aplacing', 'GP_09', 'AV_09'. 'GP_08', 'AV_08', 'GP_07', 'AV_07', 'GP_06', 'AV_06', 'GP_05', 'AV_05');
   print("<table>\n<tr>");
         ...
         while (!feof($fp)) {
            ...
            if (count($row) >= count($expectedColumns)) {
               if (!$deletePreviousBeforeCSVImport) {
                  ...
                  if ($db->query(sprintf('SELECT COUNT(*) FROM average WHERE  memberid = "%s" AND year = %s AND league = "%s"',$row[1],$row[7],$row[6])) === false) {
               ...
               if (!$deletePreviousBeforeCSVImport && $db->f(0)) {
                  ...
                  $query = sprintf('UPDATE average SET first = "%s",last = "%s",average = "%s",games = "%s",association = "%s",category = "%s",sex = "%s",oplacing = "%s",aplacing = "%s",GP_09 = "%s",AV_09 = "%s",GP_08 = "%s",AV_08 = "%s",GP_07 = "%s",AV_07 = "%s",GP_06 = "%s",AV_06 = "%s",GP_05 = "%s",AV_05 = "%s" WHERE memberid = "%s" AND year = %s AND league = "%s"',$row[3],$row[2],$row[9],$row[8],$row[0],$row[4],$row[5],$row[10],$row[11],$row[12],$row[13],$row[14],$row[15],$row[16],$row[17],$row[18],$row[19],$row[20],$row[21],$row[1],$row[7],$row[6]);
               } else {
                  /* The row does not exist in the database, so insert it */
                  $query = sprintf('INSERT INTO average (first,last,memberid,average,games,league,association,year,category,sex,oplacing,aplacing,GP_09,AV_09,GP_08,AV_08,GP_07,AV_07,GP_06,AV_06,GP_05,AV_05) VALUES ("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s")',$row[3],$row[2],$row[1],$row[9],$row[8],$row[6],$row[0],$row[7],$row[4],$row[5], $row[10],$row[11],$row[12],$row[13],$row[14],$row[15],$row[16],$row[17],$row[18],$row[19],$row[20],$row[21]) ;
               }
