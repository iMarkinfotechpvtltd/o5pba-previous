...
function display_details($topic){
	...
	foreach($items as $title=>$data){
    	echo '<li>'.$title.'(<a href="/news/posts/'.$topic->created.'/'.str_replace("+", "%20", urlencode($data[0])).'">'.$data[0].'</a>)</li>'."\n";
    }
    echo '</ul>';
}
//Draw Forms
function ask_numitems(){
	...
	?>
	<table clas="news" cellpadding="3" cellspacing="0" border="0" bgcolor="#AFAFAF">
		<tr bgcolor="#AFAFAF">
...
function add_items($numitems,$defaults=''){
	...
	if($function == 'addtopic'){
	  ...
	  <table class="news">
	  	<tr>
	  		<td>
	  			<center>
	  			<? echo getSeason('news', $_GET['time']);  ?> Season
	  			</center>
	  		</td>
	  	</tr>
	  </table>
	...
	$f->start('','', $_SERVER['PHP_SELF'].'?function='.$function);
	?>
	<table class="news" cellpadding="3" cellspacing="0" border="0" bgcolor="#AFAFAF">
	<tr bgcolor="#CFCFCF">
...
switch($function){
	...
	default:
        ...
        <table border="0" cellpadding="2" cellspacing="0" class="news">
        <?
        ...
        ?>
        <tr><td class="w20">&nbsp;</td><td class="w20">Time</td><td class="w60">Topic</td></tr>
        <?
        while($ts->next_topic()){
